...
#run_ssh_key($ssh_host, 'root', "sudo apt-get update && sudo apt-get -y install kvm libvirt-bin sysstat screen socat");
...
run_ssh_key($ssh_host,'root',"echo 'localhost:$sec_export_path $path_sec $fs_type_sec defaults,_netdev 0 0' >> /etc/fstab");
run_ssh_key($ssh_host,'root',"/bin/mount -t $fs_type_sec -o defaults,_netdev localhost:$sec_export_path $path_sec");

run_ssh_key($ssh_host,'root',"echo 'localhost:$pri_export_path $path_pri $fs_type_pri defaults,_netdev 0 0' >> /etc/fstab");
run_ssh_key($ssh_host,'root',"/bin/mount -t $fs_type_pri -o defaults,_netdev localhost:$pri_export_path $path_pri");
